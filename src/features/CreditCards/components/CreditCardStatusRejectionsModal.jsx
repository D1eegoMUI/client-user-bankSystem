...
export const CreditCardStatusRejectionsModal = ({ isOpen, onClose, requests = [] }) => {
    ...
    return (
        <BaseModal
            isOpen={isOpen}
            onClose={onClose}
            title="Cambios de Estado Rechazados"
            subtitle="Historial de solicitudes de activación / desactivación rechazadas"
            maxWidth="max-w-lg"
        >
            {requests.length === 0 ? (
                <div className="py-10 text-center text-gray-400 font-medium italic text-sm">
                    No hay rechazos de cambio de estado para esta tarjeta.
                </div>
            ) : (
